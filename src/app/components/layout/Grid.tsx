...
export function Container({
  ...
  return (
    <div
      className={`mx-auto w-full max-w-[1440px] px-[20px] md:px-[44px] lg:px-[60px] ${className}`}
    >
      {children}
    </div>
  );
...
export function Grid({ children, className = "" }: { children: ReactNode; className?: string }) {
  return (
    <div
      className={`grid grid-cols-4 gap-[16px] md:grid-cols-8 md:gap-[22px] lg:grid-cols-12 lg:gap-[24px] ${className}`}
    >
      {children}
    </div>
  );
